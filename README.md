...
- **Briefing cards** — an optional GOAL / STATE / NEXT summary of each session (generated by your own `claude -p`), so you know where you left off before re-entering.
- **Preview tabs, stars & pins** — browse without clutter (a single preview tab until you engage), and keep important sessions and projects at the top.
- **A real terminal** — xterm.js over a PTY, with full input-method support (works with pinyin and other IMEs).
